...
export async function leadCandidates(nowIso) {
  ...
  try {
    ...
    const { data: plans } = await c.from('scope_plans')
      .select('prospect_id, keys, segment, total_lo, total_hi, created_at')
      .in('prospect_id', ids).order('created_at', { ascending: false });
    // most-recent plan per prospect — so the email can name what they actually scoped
    const planFor = {};
    (plans || []).forEach((r) => { if (!planFor[r.prospect_id]) planFor[r.prospect_id] = r; });
    const out = list.filter((p) => !withProposal.has(p.id))
      .map((p) => ({ prospect: p, hasPlan: !!planFor[p.id], plan: planFor[p.id] || null }));
    return { ok: true, data: out };
  } catch (e) { return { ok: false, error: String((e && e.message) || e) }; }
}
// Cold-outbound candidates: sourced prospects (source 'outbound', still at stage 'new',
// contactable + not opted out) plus their prior outbound_* sends so outboundDueStep() can
// decide which touch is due. Capped; the cron applies the send cap on top.
export async function outboundCandidates(nowIso) {
  if (!isEnabled()) return { ok: false, skipped: true };
  try {
    const c = client();
    const { data: prospects } = await c.from('scope_prospects')
      .select('id,email,name,qualification,stage,source,unsubscribed,nurture_suppressed,created_at,updated_at')
      .eq('stage', 'new').eq('source', 'outbound')
      .eq('unsubscribed', false).eq('nurture_suppressed', false)
      .not('email', 'is', null).limit(500);
    const listP = prospects || [];
    if (!listP.length) return { ok: true, data: [] };
    const ids = listP.map((p) => p.id);
    const { data: sends } = await c.from('scope_nurture_sends')
      .select('prospect_id, step, sent_at').in('prospect_id', ids)
      .in('step', ['outbound_1', 'outbound_2', 'outbound_3']);
    const byProspect = {};
    (sends || []).forEach((s) => { (byProspect[s.prospect_id] = byProspect[s.prospect_id] || []).push({ step: s.step, sent_at: s.sent_at }); });
    return { ok: true, data: listP.map((p) => ({ prospect: p, sends: byProspect[p.id] || [] })) };
  } catch (e) { return { ok: false, error: String((e && e.message) || e) }; }
}

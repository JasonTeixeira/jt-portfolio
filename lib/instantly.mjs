...
// List the workspace's sending inboxes (the ones you bought/connected in Instantly).
export async function listAccounts(limit = 100) { return call('GET', `/accounts?limit=${limit}`); }

// Attach sending inboxes to a campaign + set the campaign-wide daily send cap.
export async function attachAccounts(campaignId, emails, dailyLimit = 50) {
  return call('PATCH', `/campaigns/${campaignId}`, { email_list: emails, daily_limit: dailyLimit });
}

export async function activateCampaign(id) { return call('POST', `/campaigns/${id}/activate`); }
export async function pauseCampaign(id) { return call('POST', `/campaigns/${id}/pause`); }

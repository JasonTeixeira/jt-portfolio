...
// Upsert a suppression. Idempotent on email; keeps the earliest created_at.
export const suppress = (email, reason, detail = {}) => {
  ...
  return guard((c) => c.from('scope_email_suppressions')
    ...
    .select('email,reason').then((r) => (Array.isArray(r.data) ? r.data[0] : r.data)));
};

export const unsuppress = (email) => {
  const e = norm(email);
  if (!e) return Promise.resolve({ ok: false, error: 'no_email' });
  return guard((c) => c.from('scope_email_suppressions').delete().eq('email', e).then(() => true));
};

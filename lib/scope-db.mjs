...
export const upsertOutboundProspect = ({ email, name, company, title, score, tier, reason, opener, verifyStatus, phone, vertical, channel, automations } = {}) =>
  guard(async (c) => {
    const em = String(email || '').trim().toLowerCase();
    ...
    const qualification = {
      ...
      opener: opener ? String(opener).slice(0, 700) : null,
      // The tailored proposal: which automations we'd pitch THIS business (id + name), capped.
      automations: Array.isArray(automations)
        ? automations.slice(0, 6).map((a) => ({ id: String((a && a.id) || '').slice(0, 40), name: String((a && a.name) || '').slice(0, 80) })).filter((a) => a.id)
        : null,

...
export function scopePlanEmail({ segmentLabel, phases, totalBand, timelineWeeks, bookUrl, planUrl, disclaimer, auditPrice = 497 }) {
  ...
  const weeks = (Array.isArray(timelineWeeks) && timelineWeeks[1]) ? `~${timelineWeeks[0]}–${timelineWeeks[1]} weeks` : '';
  ...
  const auditRung =
    `<tr><td style="padding:2px 0 16px"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#EEFBF5;border:1px solid ${GREEN};border-radius:12px"><tr><td style="padding:15px 18px">` +
      `<table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr>` +
        `<td style="font-family:'JetBrains Mono',monospace;font-size:11px;letter-spacing:.08em;text-transform:uppercase;color:${GREEN};font-weight:700">Start here</td>` +
        `<td align="right" style="font-size:19px;font-weight:700;color:${GREEN}">${esc(auditUsd)}</td>` +
      `</tr></table>` +
      `<div style="font-size:15px;font-weight:700;color:${INK};margin-top:7px">${esc(auditUsd)} AI Quality Audit</div>` +
      `<div style="font-size:13px;line-height:1.55;color:${INK};margin-top:5px">A fixed-price audit week: I dig into your setup and hand you a written plan + a firm quote. It's <strong>credited into the build</strong>, so the real cost of starting is <strong>$0</strong>.</div>` +
    `</td></tr></table></td></tr>`;
  const bodyRows =
    para(`Here's the plan you just scoped${segmentLabel ? ` for your ${esc(segmentLabel).toLowerCase()}` : ''}. Everything below is grounded in a fixed rate card, so these ranges are honest, not made up on the spot.`) +
    auditRung +
    phaseRows +
    amountRow('Full build (indicative)', range(totalBand), true) +
    (midpoint ? amountRow('Typical', `~${usd(midpoint)}`) : '') +
    (weeks ? amountRow('Rough timeline', weeks) : '') +
    para(`<span style="color:${MUTED};font-size:13px">${esc(disclaimer || 'Indicative ranges, not a quote. Exact scope and price are locked on a short call.')}</span>`) +
    para(`<b>What happens next:</b> start with the ${esc(auditUsd)} audit above — it's credited into the build, so the real cost of starting is $0 — or book a 15-minute call below and I'll turn this into a firm proposal: fixed scope, fixed price, a start date. No obligation, and I only take one engagement at a time so you'll always talk to me, not a queue.`) +
    (isHttp(planUrl) ? para(`<span style="color:${MUTED};font-size:12px">Your shareable plan link: <a href="${esc(planUrl)}" style="color:${GREEN}">${esc(planUrl)}</a></span>`) : '');
  return {
    subject: `Your scoped plan${range(totalBand) ? ` — ${range(totalBand)}` : ''}`,
    text: `Here's the plan you just scoped.\n\nSTART HERE — ${auditUsd} AI Quality Audit\nA fixed-price audit week: a written plan + a firm quote, credited into the build — so the real cost of starting is $0.\n\n${(phases || []).map((p) => `${p.label}\n${(p.items || []).map((i) => `  - ${i.name} (${range(i.band)})`).join('\n')}`).join('\n\n')}\n\nFull build (indicative): ${range(totalBand)}${midpoint ? `\nTypical: ~${usd(midpoint)}` : ''}${weeks ? `\nRough timeline: ${weeks}` : ''}\n\n${disclaimer || 'Indicative ranges, not a quote. Exact scope and price are locked on a short call.'}\n\nStart with the ${auditUsd} audit (credited into the build) or book a 15-minute call: ${bookUrl}\n\n— ${SIGNOFF}`,
    html: shell({ preheader: `Start here — ${auditUsd} AI Quality Audit, credited into the build`, heading: 'Your scoped plan', bodyRows, ctaLabel: 'Book a 15-min call', ctaUrl: bookUrl }),
  };
}

...
export const recentActivity = (limit = 24) =>
  guard(async (c) => {
    ...
    // Over-fetch, then drop anonymous-telemetry events (their prospect has no email)
    // so the Overview feed shows real contactable activity, not bot page-loads.
    const evs = await c.from('scope_events')
      .select('id,type,meta,created_at,scope_prospects(name,email)')
      .order('created_at', { ascending: false }).limit(cap * 3).then(list);
    return evs.filter((e) => e.scope_prospects && e.scope_prospects.email).slice(0, cap);
  });

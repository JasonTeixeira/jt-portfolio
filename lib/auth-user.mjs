...
// Read the `aal` (assurance level: 'aal1' | 'aal2') from a JWT we've ALREADY validated
// via the /auth/v1/user round-trip. Decode-only (no signature check) is safe here because
// the token was just proven valid by Supabase; we only need the claim value.

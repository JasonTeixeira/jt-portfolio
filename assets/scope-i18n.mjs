...
export const SCOPE_I18N = {
  es: {
    ...
    phases: { audit: 'Auditoría', build: 'Desarrollo', gate: 'Validación', operate: 'Operación' },
    segments: {
    ...
    questions: {
      ...
      needs: {
        ...
        options: {
          ...
          'opt-voice': 'Responder cada llamada automáticamente',
    ...
    cards: {
      ...
      'llm-eval': { name: 'Harness de evaluación de LLM', why: 'Conjunto de referencia (golden set) + puntuación con LLM como juez para tu función.' },
      redteam: { name: 'Red-team de IA y batería de seguridad', why: 'Pruebas de inyección, jailbreak, PII y toxicidad.' },
      'ci-gate': { name: 'Gate de calidad de IA en CI', why: 'Un cambio de IA defectuoso bloquea el merge, no la retro.' },
      grounding: { name: 'Gate de alucinación / grounding', why: 'Evita que invente hechos y políticas.' },
      regression: { name: 'Pruebas de regresión de prompts y modelos', why: 'Sabe exactamente qué rompió la actualización del modelo.' },
      ...
      visual: { name: 'Pruebas de regresión visual', why: 'Detecta el fallo de maquetación que una prueba unitaria no ve.' },
      ...
      'lead-capture': { name: 'Captura de prospectos → calificar → enrutar', why: 'Cada prospecto captado, calificado y contactado en minutos.' },
      etl: { name: 'Pipelines de datos / ETL', why: 'Mueve y transforma datos de forma confiable, de forma programada.' },
  ...
  pt: {
    effortUnit: 'sem',
    disclaimer: 'Faixas indicativas, não um orçamento. O escopo e o preço exatos são definidos em uma breve chamada.',
    phases: { audit: 'Auditoria', build: 'Desenvolvimento', gate: 'Validação', operate: 'Operação' },
    segments: {
    ...
    cards: {
      ...
      rag: { name: 'Engenharia de pipeline RAG', why: 'Recuperação que retorna o certo, com citações.' },
      ...
      'llm-eval': { name: 'Harness de avaliação de LLM', why: 'Golden set + pontuação com LLM como juiz para o seu recurso.' },
      redteam: { name: 'Red-team de IA e bateria de segurança', why: 'Testes de injeção, jailbreak, PII e toxicidade.' },
      'ci-gate': { name: 'Gate de qualidade de IA no CI', why: 'Uma mudança de IA ruim bloqueia o merge, não a retrô.' },
      ...
      cicd: { name: 'Pipeline CI/CD + integração de testes', why: 'Um selo verde em que você pode confiar.' },
      flaky: { name: 'Estabilização de testes instáveis', why: 'Faça o vermelho voltar a significar algo.' },
      perf: { name: 'Baselines de desempenho e carga', why: 'Conheça o ponto de ruptura do seu caminho crítico.' },
      ...
      etl: { name: 'Pipelines de dados / ETL', why: 'Mova e transforme dados de forma confiável, de forma agendada.' },

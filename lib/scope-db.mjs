...
export const upsertOutboundProspect = ({ email, name, company, title, score, tier, reason, opener, verifyStatus } = {}) =>
  guard(async (c) => {
    const em = String(email || '').trim().toLowerCase();
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(em)) throw new Error('valid email required');
    const qualification = {
      source: 'outbound', score: Number.isFinite(score) ? Math.max(0, Math.min(100, Math.round(score))) : null,

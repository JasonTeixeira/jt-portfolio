...
/**
 * Resolve which tenant an inbound message/call belongs to, by the RECEIVING
 * number. Fail-closed: with a number map configured (multi-tenant), an unmapped
 * number returns { matched:false } so we NEVER answer as / route a real client's
 * customer to the demo owner. With no map configured (single demo number), we
 * default to the demo tenant. `toNumber` is the number that was texted/called.
 */
export function resolveTenant(toNumber) {
  const map = numberMap();
  if (!Object.keys(map).length) {
    return { key: DEMO_TENANT, tenant: TENANTS[DEMO_TENANT], matched: true };
  }
  const key = map[normalize(toNumber)];
  if (!key || !TENANTS[key]) return { key: null, tenant: null, matched: false };
  return { key, tenant: TENANTS[key], matched: true };
}

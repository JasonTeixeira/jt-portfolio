...
import { arAging } from '../lib/invoice-db.mjs';
...
async function handler(req, res) {
  ...
  const [rev, stages, aging] = await Promise.all([revenueSummary(), prospectStageCounts(), arAging()]);
  ...
  return res.status(200).json({ ok: true, revenue: rev.data, stages: stages.ok ? stages.data : {}, aging: aging.ok ? aging.data : null });
}

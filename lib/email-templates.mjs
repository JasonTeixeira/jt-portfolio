...
export function scopePlanEmail({ segmentLabel, phases, totalBand, timelineWeeks, bookUrl, planUrl, disclaimer }) {
  ...
  const bodyRows =
    ...
    phaseRows +
    amountRow('Indicative total', range(totalBand), true) +
    ...
    para(`<b>What happens next:</b> book a 15-minute call below and I'll turn this into a firm proposal — fixed scope, fixed price, a start date. No obligation, and I only take one engagement at a time so you'll always talk to me, not a queue.`) +

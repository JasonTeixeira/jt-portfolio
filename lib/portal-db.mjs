...
export const listClientProjectsByEmail = (email) =>
  guard((c) => c.from('scope_projects')
    .select('id,status,portal_token,created_at,scope_proposals!inner(id,public_id,client_email,firm_cents,deposit_cents,balance_cents,paid_at,balance_paid_at,keys,segment)')
    .eq('scope_proposals.client_email', String(email).toLowerCase())
    .order('created_at', { ascending: false }).then(list));

// Attention-band aggregates for the client dashboard — batched over the client's own
// project ids (never N+1). Return raw {project_id} rows; the caller groups + counts.
export const deliveredMilestonesByProjects = (projectIds) =>
  guard((c) => {
    if (!Array.isArray(projectIds) || !projectIds.length) return [];
    return c.from('scope_milestones').select('project_id').eq('status', 'delivered').in('project_id', projectIds).then(list);
  });
export const unreadClientMessagesByProjects = (projectIds) =>
  guard((c) => {
    if (!Array.isArray(projectIds) || !projectIds.length) return [];
    return c.from('scope_messages').select('project_id').eq('sender', 'operator').is('read_by_client_at', null).in('project_id', projectIds).then(list);
  });
...
// Client message attachments live in their own private bucket with a hard 25MB storage-level
// ceiling (see scope_schema.sql) — smaller than deliverables and isolated from operator files.
const MESSAGE_BUCKET = 'message-uploads';
...
// download:true forces Content-Disposition: attachment, so an uploaded file can never render
// inline in the browser (neutralizes a malicious HTML/SVG serving script).
async function signBucketDownload(bucket, storagePath, expiresIn = 300) {
  if (!isEnabled()) return { ok: false, skipped: true };
  try {
    const { data, error } = await client().storage.from(bucket).createSignedUrl(storagePath, expiresIn, { download: true });
    if (error) return { ok: false, error: error.message };
    return { ok: true, url: data.signedUrl };
  } catch (e) { return { ok: false, error: String((e && e.message) || e) }; }
}
export const signDeliverableDownload = (storagePath, expiresIn = 300) => signBucketDownload(DELIVERABLES_BUCKET, storagePath, expiresIn);
// Message attachments live in their own bucket — sign against it, not deliverables.
export const signMessageDownload = (storagePath, expiresIn = 300) => signBucketDownload(MESSAGE_BUCKET, storagePath, expiresIn);
...
export const listMessages = (projectId, limit = 200) =>
  guard((c) => c.from('scope_messages').select('id,sender,body,created_at,attachment_path,attachment_name,attachment_size,attachment_type')
...
// An optional attachment {path,name,size,type} is stored by reference; the file itself lives
// in the private bucket and is only ever served via a short-lived signed download URL.
export const addMessage = (projectId, sender, body, attachment) =>
  guard((c) => c.from('scope_messages')
    .insert({ project_id: projectId, sender, body: String(body).slice(0, 5000),
      attachment_path: attachment && attachment.path ? String(attachment.path).slice(0, 300) : null,
      attachment_name: attachment && attachment.name ? safeName(attachment.name) : null,
      attachment_size: attachment && typeof attachment.size === 'number' ? attachment.size : null,
      attachment_type: attachment && attachment.type ? String(attachment.type).slice(0, 120) : null,
    })
    .select().then(first));

// Pure: accept a client-supplied attachment ref ONLY if its path was minted by this project's
// own sign_upload (prefix `msg/<projectId>/`). Anything else → null, so a client can never
// reference another project's or an arbitrary bucket object. Metadata is bounded/sanitized.
export function normalizeMessageAttachment(projectId, att) {
  if (!projectId || !att || typeof att !== 'object') return null;
  const path = typeof att.path === 'string' ? att.path : '';
  if (!path.startsWith(`msg/${projectId}/`)) return null;
  if (path.includes('..')) return null; // defensive: no traversal segments even after a valid prefix
  return {
    path: path.slice(0, 300),
    name: att.name ? safeName(att.name) : null,
    size: typeof att.size === 'number' && att.size >= 0 ? att.size : null,
    type: att.type ? String(att.type).slice(0, 120) : null,
  };
}

// Client (or operator) asks for a one-time signed upload URL for a message attachment. Path
// is server-built under the project's own `msg/` prefix — the caller can't choose it, so no
// traversal and no cross-project write.
export async function signMessageUpload(projectId, filename) {
  if (!isEnabled()) return { ok: false, skipped: true };
  try {
    const path = `msg/${projectId}/${globalThis.crypto.randomUUID()}-${safeName(filename)}`;
    const { data, error } = await client().storage.from(MESSAGE_BUCKET).createSignedUploadUrl(path);
    if (error) return { ok: false, error: error.message };
    return { ok: true, path, token: data.token, signedUrl: data.signedUrl };
  } catch (e) { return { ok: false, error: String((e && e.message) || e) }; }
}
...
// ---- Operator inbox (cross-project) ----------------------------------------
// Every client->operator message the operator hasn't read yet, joined to enough
// project/proposal context to render a thread list. One query; the caller groups
// by project into threads + a total unread badge count.
export const operatorUnreadMessages = () =>
  guard((c) => c.from('scope_messages')
    .select('id,project_id,body,created_at,scope_projects!inner(portal_token,scope_proposals!inner(public_id,client_email))')
    .eq('sender', 'client').is('read_by_operator_at', null)
    .order('created_at', { ascending: false }).limit(500).then(list));

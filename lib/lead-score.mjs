...
export function ruleScoreBusiness(biz = {}) {
  ...
  const benefit = (picks[0] && picks[0].outcome) || 'answers every call and books the work automatically';
  // Human, varied openers — chosen deterministically by the business name so no two neighbours
  // get the same line, and each reads like a person wrote it (not a template). The email wrapper
  // adds the automation list + soft CTA around this hook.
  const OPENERS = [
    (n, k, b) => `Hi — I came across ${n} and had a quick thought. A lot of ${k} businesses quietly lose work when calls go unanswered or a lead sits overnight. I set up AI that handles that in the background — it ${b}.`,
    (n, k, b) => `Hey — noticed ${n} while looking at ${k} businesses in your area. The ones doing best right now never miss a call or let a lead go cold. That's the kind of thing I build: AI that ${b}, all on autopilot.`,
    (n, k, b) => `Hi — quick one for ${n}. Most ${k} owners I talk to are losing a few jobs a month to missed calls and slow follow-up without even realizing it. I put together AI that quietly fixes that — it ${b}.`,
    (n, k, b) => `Hi — I help ${k} businesses like ${n} stop leaving money on the table from missed calls and leads that go cold. It's AI that ${b}, running quietly in the background so you don't have to think about it.`,
    (n, k, b) => `Hey — reaching out because ${n} looks like exactly the kind of ${k} business this helps. I set up AI that ${b} — so nothing slips through when you're busy on a job or it's after hours.`,
  ];
  let h = 0; for (const c of nm) h = (h * 31 + c.charCodeAt(0)) >>> 0;
  const pitch = OPENERS[h % OPENERS.length](nm, v.kind, benefit);
  return {

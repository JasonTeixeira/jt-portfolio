...
import { userFromRequest, isAdminEmail, emailFromToken } from './auth-user.mjs';
...
// Who is acting, for the audit log. The break-glass token is unattributable; a JWT operator
// is labeled by their (already-validated) email. Cheap — decode-only, no network call.
export function adminActor(req) {
  if (checkToken(req)) return 'break-glass';
  const h = (req.headers && (req.headers.authorization || req.headers.Authorization)) || '';
  const tok = h.startsWith('Bearer ') ? h.slice(7).trim() : '';
  return emailFromToken(tok) || 'operator';
}

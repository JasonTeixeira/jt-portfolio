...
// Physical postal address — required in every commercial email by CAN-SPAM §5.
// Matches the nurture footer (assets/nurture-core.mjs) so all marketing mail is consistent.
const POSTAL = 'Sage Ideas LLC · Orlando, FL';
const INK = '#1A1A1A';
const MUTED = '#6B6B6B';
const GREEN = '#0F9D6C';
const LINE = '#E7E4DF';
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${BG};padding:28px 12px"><tr><td align="center">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;background:#ffffff;border:1px solid ${LINE};border-radius:16px;overflow:hidden">
    ...
    <tr><td style="border-top:1px solid ${LINE};padding:18px 28px;font-size:12px;color:${MUTED}">
      ${esc(SIGNOFF)}<br>You're getting this because you subscribed to field notes from ${esc(BRAND)}.${unsub ? ` <a href="${esc(unsub)}" style="color:${MUTED}">Unsubscribe</a>.` : ''}<br><span style="color:${MUTED}">${esc(POSTAL)}</span>
    </td></tr>
  </table>

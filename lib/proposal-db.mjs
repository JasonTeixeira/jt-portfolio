...
// Idempotency guard for the public draft endpoint: a double-submit (double-click / retry)
// must not create a second draft + email the operator twice. Returns the public_id of an
// existing DRAFT for this prospect with the SAME scope (keys+segment) created within
// `withinMs`, or null. Compared in JS because `keys` is an array column.
export const findRecentDraftPublicId = (prospectId, keys, segment, withinMs = 10 * 60 * 1000) =>
  guard(async (c) => {
    const since = new Date(Date.now() - withinMs).toISOString();
    const recent = await c.from('scope_proposals')
      .select('public_id,keys,segment,created_at')
      ...
      .limit(10)
      .then(list);
    const norm = (k, s) => JSON.stringify([...(Array.isArray(k) ? k : [])].sort()) + '|' + (s || '');
    const want = norm(keys, segment);
    const hit = recent.find((r) => norm(r.keys, r.segment) === want);
    return hit ? hit.public_id : null;
  });

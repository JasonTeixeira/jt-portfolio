...
// Cross-prospect activity feed for the Overview. Left-joins the prospect for context so
// events with no prospect (e.g. disputes) still show. Newest first.
export const recentActivity = (limit = 24) =>
  guard((c) => c.from('scope_events')
    .select('id,type,meta,created_at,scope_prospects(name,email)')
    .order('created_at', { ascending: false }).limit(Math.min(Math.max(1, limit | 0), 60)).then(list));
...
export const newLeadsWeekDelta = () =>
  guard(async (c) => {
    ...
    const rows = await c.from('scope_prospects').select('created_at').gte('created_at', prevMon.toISOString()).then(list);

...
${(s.packages || []).map((p, i) => `      <div style="border:1px solid ${i === 0 ? s.color : '#2A2826'};background:#0C0C0E;padding:26px;display:flex;flex-direction:column;gap:12px">
    ...
    <p style="margin:18px 0 0;${MONO}font-size:11px;color:#837D77">fixed-price starting points from $497, plus a custom quote for larger or unusual builds — scoped in writing before we start, so you pay for your problem, not a package · every engagement ends with evidence you keep — and if the scoping shows I can’t help, I’ll say so and it costs nothing</p>
  </section>
